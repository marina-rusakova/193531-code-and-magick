...
window.renderStatistics = function (ctx, names, times) {
  ...
  ctx.fillRect(cloudShadowX, window.convertY(cloudShadowY), cloudWidth, cloudHeight);

  ctx.fillStyle = 'white';
  ctx.fillRect(cloudX, window.convertY(cloudY), cloudWidth, cloudHeight);
  ...
  ctx.fillText('Ура вы победили!', cloudTitleX, window.convertY(cloudTitleY));
  ctx.fillText('Список результатов:', cloudTitleX, window.convertY(cloudTitle2Y));
...
var Histogram = {
  ...
  drawColumn: function (ctx, name, time, playerNumber, ratio, x0, y0) {
    ...
    ctx.fillRect(columnX, window.convertY(columnY), this.histWidth, columnHeight);

    ctx.fillStyle = 'black';
    ctx.font = '16px PT Mono';
    ctx.textBaseline = 'hanging';
    ctx.fillText(name, columnX, window.convertY(this.lineHeight + y0));
    ctx.fillText(Math.round(time), columnX, window.convertY(this.lineHeight + this.lineSpace + columnY));
  },
